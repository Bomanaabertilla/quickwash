  ...
  return (
    <nav className="absolute bottom-0 left-0 right-0 h-[72px] bg-white border-t border-slate-200 flex items-center justify-around z-30 pb-1">
      ...
      <button
        ...
        className={`flex flex-col items-center gap-1 px-4 py-1.5 rounded-lg text-xs font-semibold transition-colors ${
          activeTab === 'laundry' ? 'text-brand-teal' : 'text-slate-500 hover:text-slate-800'
        }`}
      >
        <svg className={`w-5 h-5 ${activeTab === 'laundry' ? 'text-brand-teal stroke-[2.3]' : 'text-slate-500'}`} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      ...
      >
        <FileText className={`w-5 h-5 ${activeTab === 'orders' ? 'text-brand-teal stroke-[2.3]' : 'text-slate-500'}`} />
        <span>Orders</span>
      </button>
      ...
      >
        <Shirt className={`w-5 h-5 ${activeTab === 'pricing' ? 'text-brand-teal stroke-[2.3]' : 'text-slate-500'}`} />
        <span>Pricing</span>
      </button>
    </nav>
  );

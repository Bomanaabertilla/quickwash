...
import { getPublishedAvailability, savePublishedAvailability } from '../data/availabilityStore';
import { getCurrentWeekLabel, getCalendarWeek } from '../utils/dateUtils';
...
export interface OwnerWeeklyAvailabilityScreenProps {
  onShowToast?: (msg: string) => void;
  onPreviewCustomerView?: () => void;
  onBackToDashboard?: () => void;
}

export default function OwnerWeeklyAvailabilityScreen({
  onShowToast,
  onPreviewCustomerView
}: OwnerWeeklyAvailabilityScreenProps) {
  // Load initial availability state from storage or default
  const [availabilityState, setAvailabilityState] = useState<WeeklySchedule>(() => getPublishedAvailability());
  const [isPublished, setIsPublished] = useState<boolean>(() => availabilityState.isPublished ?? true);
  const [isUnsavedChanges, setIsUnsavedChanges] = useState(false);

  // 7 Days state
  const [days, setDays] = useState<DaySchedule[]>(() => availabilityState.days || []);

  // Modal for editing a specific day's slots & hours
  const [selectedDayModal, setSelectedDayModal] = useState<DaySchedule | null>(null);
  ...
  const handleCopyPreviousWeek = () => {
    const updated: DaySchedule[] = days.map(d => {
      if (d.id === 'sun') {
        return {
          ...d,
          status: 'Closed',
      ...
      return {
        ...
        slots: d.id === 'sat' ? [
          { time: '9:00 AM', period: 'Morning', status: 'Available', capacity: 4, bookedCount: 0 },
          { time: '11:30 AM', period: 'Morning', status: 'Available', capacity: 4, bookedCount: 0 },
          { time: '2:00 PM', period: 'Afternoon', status: 'Available', capacity: 4, bookedCount: 0 },
          { time: '4:30 PM', period: 'Evening', status: 'Available', capacity: 4, bookedCount: 0 }
        ] : [
          { time: '8:00 AM', period: 'Morning', status: 'Available', capacity: 5, bookedCount: 0 },
          { time: '10:00 AM', period: 'Morning', status: 'Available', capacity: 5, bookedCount: 0 },
          { time: '1:00 PM', period: 'Afternoon', status: 'Available', capacity: 5, bookedCount: 0 },
          { time: '3:30 PM', period: 'Afternoon', status: 'Available', capacity: 5, bookedCount: 0 },
          { time: '5:00 PM', period: 'Evening', status: 'Available', capacity: 5, bookedCount: 0 }
        ]
      };
    });
    setDays(updated);
  ...
  const handleToggleDay = (dayId?: string | number) => {
    if (!dayId) return;
    const next: DaySchedule[] = days.map(d => {
  ...
  const handleUnpublishSchedule = () => {
    const weekInfo = getCalendarWeek();
    const closedDays: DaySchedule[] = days.map(d => {
      const calDay = weekInfo.days.find(c => c.id === d.id);
      return {
        ...d,
        date: calDay?.date || d.date,
  ...
  const handleRemoveSlotFromModal = (slotTime: string) => {
    if (!selectedDayModal) return;
    setSelectedDayModal({
      ...selectedDayModal,
  ...
  return (
    <div className="flex-1 flex flex-col bg-[#FAF9F5] text-slate-800 pb-28 overflow-y-auto">
      ...
      <div className="px-6 lg:px-10 max-w-7xl mx-auto w-full flex flex-col gap-8 mt-2">
        ...
        <Card className="rounded-3xl border-slate-200/90 shadow-2xs p-6 lg:p-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-8 bg-white">
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="flex items-center justify-between flex-wrap gap-2">
            <div className="flex items-center gap-3">
              ...
              <Badge variant={totalOpenDays > 0 ? 'success' : 'secondary'} className="text-xs font-bold px-3 py-0.5 rounded-full">
                {totalOpenDays > 0 
            ...
            {days.map((day) => {
              ...
              return (
                <Card
                  ...
                  className={cn(
                    'p-4 sm:p-5 flex flex-col justify-between min-h-[220px] transition-all cursor-pointer shadow-2xs hover:shadow-sm',
                    isOpen
                ...
                >
                  ...
                  <div>
                    <div className="flex items-center justify-between">
                      ...
                      <span className={cn(
                        ...
                        isOpen ? 'bg-emerald-100 text-emerald-800' : 'bg-[#e2e8f0] text-slate-700'
                      )}>
                        {day.status}
                      </span>
                    </div>
                  ...
                  <div className="pt-3 border-t border-slate-100/80 mt-4">
                    <button
                      ...
                      className={cn(
                        'w-full py-2 rounded-xl text-xs font-bold flex items-center justify-center gap-1.5 transition-colors cursor-pointer',
                        isOpen
      ...
      <div className="fixed bottom-4 left-1/2 -translate-x-1/2 z-40 bg-white/95 backdrop-blur-md rounded-3xl border border-slate-200/90 shadow-2xl p-4 flex items-center gap-4 flex-wrap max-w-2xl w-[92%] justify-between">
        <div className="flex items-center gap-3">
          <div className={cn('w-3 h-3 rounded-full', isPublished && totalOpenDays > 0 ? 'bg-emerald-500 animate-ping' : 'bg-amber-400')} />
          <div>
      ...
      {selectedDayModal && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-xs flex items-center justify-center p-4 z-[999] animate-fade-in">
          <Card className="rounded-3xl max-w-md w-full shadow-2xl border-slate-200 p-6 flex flex-col gap-4 max-h-[90vh] overflow-y-auto bg-white">
            ...
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <div>
                <div className="flex items-center gap-2">
                  ...
                  <Badge variant={selectedDayModal.status === 'Open' ? 'success' : 'secondary'} className="text-[10px] font-bold px-2 py-0.5">
                  ...
                  onClick={() => {
                    ...
                    setSelectedDayModal({
                      ...
                      slots: willOpen && (!selectedDayModal.slots || selectedDayModal.slots.length === 0) ? [
                        ...
                        { time: '10:00 AM', period: 'Morning', status: 'Available', capacity: 5, bookedCount: 0 },
                        { time: '1:00 PM', period: 'Afternoon', status: 'Available', capacity: 5, bookedCount: 0 },
                        { time: '4:00 PM', period: 'Evening', status: 'Available', capacity: 5, bookedCount: 0 }
                      ] : selectedDayModal.slots
                    });
                  }}
                  className={cn(
                    'px-3 py-1 rounded-xl text-xs font-bold cursor-pointer transition-colors',
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-2">
                      <Input
                        ...
                        className="flex-1 bg-white border-slate-300 text-xs font-bold text-slate-800 focus:border-[#008276]"
                      />
                      <Button
                        type="button"
                        variant="secondary"
                        ...
                        className="bg-teal-50 hover:bg-teal-100 text-[#008276] text-xs font-bold cursor-pointer"
                      >
                        + Add Slot
                      </Button>
                    </div>

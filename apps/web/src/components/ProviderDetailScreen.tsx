...
import type { Partner, WeeklySchedule, ShopProfile, DaySchedule, TimeSlot } from '@quickwash/shared';

export interface ProviderDetailScreenProps {
  partner?: Partner | null;
  onBack: () => void;
  onContinueToServices: (slotDesc: string) => void;
  onSupportClick?: () => void;
  onProfileClick?: () => void;
}
...
export default function ProviderDetailScreen({
  ...
  const [publishedSchedule, setPublishedSchedule] = useState<WeeklySchedule>(() => getPublishedAvailability());
  ...
  const [selectedSlot, setSelectedSlot] = useState(() => {
    ...
    const firstSlot = today?.slots?.[0] || { time: '10:00 AM', period: 'Morning', status: 'Available', capacity: 10 };
    return {
      time: firstSlot.time,
      date: `${today?.name || 'Today'} ${today?.date || ''}`,
      period: firstSlot.period || 'Morning'
    };
  });
  ...
  const activeDay = useMemo<DaySchedule | null>(() => {
  ...
  const slotsData = useMemo<SlotSection[]>(() => {
  ...
  const handleSelectSlot = (slotTime: string, period?: string) => {
    if (!activeDay) return;
    setSelectedSlot({
      time: slotTime,
      date: `${activeDay.name || ''} ${activeDay.date || ''}`,
      period: (period as 'Morning' | 'Afternoon' | 'Evening') || 'Morning'
    });
  };
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-[#f7f9fc]">
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar px-5 pt-3 pb-24 flex flex-col gap-4">
        ...
        {isOffline && (
          <div className="bg-[#fffbeb] border border-[#fce6a0] rounded-2xl p-3.5 flex items-center justify-between gap-3 shadow-xs">
            <div className="flex items-start gap-3">
              ...
              <div className="flex flex-col text-xs">
                ...
                <p className="text-amber-800 text-[11px] font-medium mt-0.5 leading-relaxed">
                  Viewing cached schedule. Live availability will sync when you reconnect.
                </p>
              </div>
            </div>
            ...
            <Button
              ...
              disabled={isRetrying}
              className="bg-amber-100 hover:bg-amber-200 text-amber-900 font-bold text-xs px-3 py-1.5 rounded-xl flex items-center gap-1.5 flex-shrink-0 transition-colors border-amber-200 cursor-pointer"
            >
        ...
        <Card className="rounded-3xl p-4 border-slate-200/80 shadow-xs flex flex-col gap-3 bg-white">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col">
              <div className="flex items-center gap-2">
                ...
                <Badge variant="secondary" className="bg-blue-100 text-blue-700 px-2 py-0.2 rounded text-[10px] font-bold border-0">
                  Standard
                </Badge>
              </div>
        ...
        {/* Horizontal Date Picker Grid */}
        <div className="grid grid-cols-7 gap-1.5 sm:gap-2">
          {publishedSchedule.days && publishedSchedule.days.map((d) => {
            const isSelected = selectedDateId === String(d.id);
            const isClosed = d.status === 'Closed';

            return (
              <button
                key={String(d.id)}
                onClick={() => setSelectedDateId(String(d.id))}
                className={cn(
                  'py-2 px-1 rounded-2xl flex flex-col items-center justify-center text-center transition-all cursor-pointer',
                  isSelected
              ...
              >
                ...
                <span className={cn(
                  'text-[9px] font-extrabold px-1 py-0.2 rounded-md',
                  isSelected 
        ...
        <div className="bg-[#f0f6ff] rounded-2xl p-3.5 flex items-center justify-between border border-blue-100 shadow-xs mt-1">
          ...
          <div className="flex items-center gap-1.5 flex-shrink-0">
            <Badge variant="warning" className="text-[10.5px] font-bold px-2 py-0.5 rounded-full flex items-center gap-1">
              <Check className="w-3 h-3 text-amber-800" />
              Cached slot
            </Badge>
            <Badge variant="success" className="text-[11px] font-bold px-2.5 py-0.5 rounded-full flex items-center gap-1">
        ...
        <Card className="rounded-2xl p-4 border-slate-200/80 shadow-xs mb-2 bg-white">
      ...
      <div className="bg-white border-t border-slate-200/80 px-5 pt-3 pb-5 z-20 shadow-lg flex items-center justify-between gap-4">
        ...
        <Button
          ...
          className="bg-[#006a60] hover:bg-[#005850] text-white px-5 py-3 rounded-2xl font-bold text-xs flex items-center gap-2 shadow-md active:scale-[0.99] transition-all flex-shrink-0 cursor-pointer"
        >
